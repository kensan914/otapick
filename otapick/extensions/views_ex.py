    ...
    def get_result(self):
        if self.group_id == 0:
            ...
            if members.exists():
                ...
                return {'status': 'not_found_member'}
            images = Image.objects.filter(
                publisher__writer__belonging_group__group_id=self.group_id, publisher__writer__ct=self.ct)
...
def sort_by_recommend_score(queryset, page, random_seed, paginate_by):
    ...
    queryset_parts = queryset[q_group_len *
                              q_group_num: q_group_len * (q_group_num + 1)]

    np.random.seed(random_seed)
    id_list = list(queryset_parts.values_list(
        'id', flat=True))  # クエリセットのidのリスト
    np.random.shuffle(id_list)
    # id_listからランダムに選んだ20個の整数リスト
    selected_id_list = id_list[paginate_by *
                               q_group_index: paginate_by * (q_group_index + 1)]
...
def sort_images_by_related(blog, order, page, paginate_by):
    ...
    last_post_year, last_post_month = divmod(
        post_month_score - 1, 12)  # 投稿日の前月の年, 投稿日の前月の月
    next_post_year, next_post_month = divmod(
        post_month_score + 1, 12)  # 投稿日の次月の年, 投稿日の次月の月
    ...
    id_list = list(Image.objects.filter(publisher=blog).exclude(
    ...
    # 該当メンバーその他
    images = base_images.exclude(id__in=id_list).filter(
        publisher__writer=blog.writer).order_by('-recommend_score', '-score')

    if images.exists():
        id_list += list(images.values_list('id', flat=True))

    # others
    images = base_images.exclude(id__in=id_list).order_by(
...
def generate_resource_data(resources, max_rank, rank_type, resource_type, prefix, modifier, suffix, group_name):
    ...
    if len(resources) >= max_rank:
        for rank in range(max_rank):
            ...
            message = prefix[rank_type] + comparison + modifier[rank_type] + \
                suffix[resource_type] + '({})'.format(group_name)
            resources_data.append({'resource': resources[rank], 'resource_info': {
                                  'type': resource_type, 'message': message}})
    else:
        return []
