...
import { URLJoin, generateAlt, isSmp, isMobile, addLongPressEventListeners, updateMeta, checkNotCached, geneIsFavoriteGetterSetter } from "../modules/utils";
...
export const downloadImage = (url, csrftoken, incrementNumOfDownloads = null, order) => {
  axios
    .post(url, {}, {
      headers: {
    ...
    return (
      <ButtonDropdown direction="right" isOpen={this.state.dropdownOpen} toggle={this.toggle}>
        ...
        <DropdownMenu className="bold">
          <DropdownItem onClick={() => downloadImage(URLJoin(BASE_URL, this.props.url), this.props.csrftoken, this.props.incrementNumOfDownloads, this.props.order)}>この画像をダウンロードする</DropdownItem>
...
class ImageView extends ViewTemplate {
  constructor(props) {
    ...
    const isFavoriteGetterSetter = geneIsFavoriteGetterSetter(props.domState.favoriteState, props.domDispatch, this.imageID);
  ...
  updateMetaVerView(status, blogTitle, blogWriter) {
    if (status === "success") {
      updateMeta({ title: `${blogTitle}(${blogWriter})｜画像詳細`, description: `${blogWriter}のブログ「${blogTitle}」の画像です。` });
      ...
      if (prevState.status !== this.state.status && this.state.status === "success") {
        if (this.state.VIEW_KEY) {
          ...
          if (!this.props.domState.accessedImages.includes(imageID)) {
            this.putView();
            this.props.domDispatch({ type: "ACCESS_TO_IMAGE", imageID: imageID });
          }
        }
        ...
        // init isFavorite
        this.props.domDispatch({ type: "INIT_FAVORITE", imageID: this.imageID, isFavorite: this.state.images[this.props.order].isFavorite });
      }
  ...
  render() {
        ...
        <div className="ml-1 ml-sm-3 ml-lg-4 image-view-text">
              ...
              {!isMobile &&
                <>
                  <Button
                    ...
                    onClick={() => {
                      image && downloadImage(URLJoin(BASE_URL, image.url), this.csrftoken, this.incrementNumOfDownloads, this.props.order);
                    }}
          ...
          <div className="image-view-footer mt-4 mt-sm-5 mt-lg-2 mb-2">
            ...
            {isMobile
              ...
              : <DetailButton officialUrl={this.state.officialUrl} url={image && image.url} incrementNumOfDownloads={this.incrementNumOfDownloads} order={this.props.order} csrftoken={this.csrftoken} />
            }
          </div>
        </div>
      );
    ...
    if (isSmp) {
      imageView = (
        <>
          <img className="image-of-image-view smp" src={src} id={this.mainImageID} alt={imageAlt} />
          ...
          <div className="card-body">
            <div className="row m-0">
              <div className="col-12 col-lg-6 p-0">
                <img className="image-of-image-view" src={src} id={this.mainImageID} srcSet={srcSet} alt={imageAlt}
