...
class SearchDownshift extends React.Component {
  ...
  onFocusInput() {
    if (this.state.qvalue) {
      if (!this.state.isOpen) {
        if (typeof this.props.navbarToggle != "undefined")
  ...
  handleSubmit(e) {
    ...
    if (this.state.qvalue) {
      this.props.history.push(
        "/search/?q=" + encodeURIComponent(this.state.qvalue)
      );
    }
  }
  ...
  documentClickHandler = (e) => {
    ...
    if (
      ...
      e.target.closest("#downshift-box") == null
    ) {
      this.endSearchWork();
    }
  };
  ...
  documentTouchmoveHandler = (e) => {
    ...
    const searchSuggestionBox = document.getElementById(
      "search-suggestions-box"
    );

    if (
      e.target.closest("#downshift-box") === downshiftBox &&
      searchSuggestionBox.scrollTop !== 0 &&
      searchSuggestionBox.scrollTop + searchSuggestionBox.clientHeight !==
        searchSuggestionBox.scrollHeight
    ) {
      e.stopPropagation();
    } else {
      if (e.cancelable) {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.isRender) {
      ...
      if (
        prevState.isOpenInit !== this.state.isOpenInit &&
      ...
      if (
        !prevState.isOpenInit &&
        !prevState.isOpen &&
        (this.state.isOpenInit || this.state.isOpen)
      ) {
      ...
      // InitDownshiftまたはDownshiftのviewが解除されたとき(検索作業が終了したとき)
        ...
        !this.state.isOpenInit &&
        !this.state.isOpen &&
        (prevState.isOpenInit || prevState.isOpen)
      ) {
      ...
      if (
        prevState.initSuggestionsStatus.length === 0 &&
        this.state.initSuggestionsStatus.length > 0
      ) {
        this.addEventListeners();
      }
      // suggestionsのデータ読み込みが完了したとき
      else if (
        prevState.suggestionsStatus.length === 0 &&
        this.state.suggestionsStatus.length > 0
      ) {
        this.addEventListeners();
      }
      ...
      if (this.props.location !== prevProps.location) {
        this.resetAll();
        if (typeof this.props.resetNavBar != "undefined")
  ...
  render() {
    ...
    if (this.state.suggestionsType === "url") {
      suggestionsMessage =
        this.state.suggestionsStatus === "success" ? (
          <div
            className="alert alert-success search-suggestions-title"
            role="alert"
            style={suggestionsMessageStyle}
          >
            有効なURLです。以下のブログが検出されました。
          </div>
        ) : (
          <NotFoundBlogsContent />
        );
    ...
    return (
      ...
      >
        {({
          ...
          <div
            className={
              "my-0 justify-content-center col " + (isMobile ? "pr-2" : "")
            }
          >
            <form
            ...
            >
              <input
                {...getInputProps()}
                className="col form-control autocomplete rounded-pill"
                type="search"
                placeholder="検索する"
              ...
              {this.state.isOpenInit && (
                ...
                >
                  ...
                  <div className={"mt-3 " + (isSmp ? "mb-2" : "mb-4")}>
                    ...
                    <hr className="mb-0" />
                    <h5 className="my-2 search-suggestions-title">
                      メンバーから画像を探す
                    </h5>
                    <div className={"row " + (isSmp ? "mx-0" : "mx-1")}>
                      {this.state.initSuggestionsMembers.length > 0 ? (
                        this.state.initSuggestionsMembers.map((item, index) => (
                          <div
                            key={index}
                            className="col-6 col-sm-4 col-md-3 col-lg-4 col-xl-3 p-0"
                          >
                            <div
                              className="m-1 search-suggestions-items"
                              {...getItemProps({
                                key: item.id,
                                index,
                                item,
                            ...
                            >
                              <div
                                className="d-flex align-items-center justify-content-center search-suggestions-items-wrapper"
                                style={{
                                  backgroundColor:
                    ...
                    <div className={"row " + (isSmp ? "mx-0" : "mx-1")}>
                      {this.state.initSuggestionsBlogs.length > 0 ? (
                        this.state.initSuggestionsBlogs.map((item, index) => (
                          <div
                            key={index}
                            className="col-6 col-sm-4 col-md-3 col-lg-4 col-xl-3 p-0"
                          >
                ...
                >
                  {isMobile && (
                    <div className="col text-right p-0">
                      <Button
                        className="rounded-circle transparent-button-mobile mt-1"
                        onClick={() => this.endSearchWork()}
                      >
                        <FontAwesomeIcon icon={faTimes} />
                      </Button>
                  ...
                  <div className={"mb-3 " + (!isMobile ? "mt-2" : "")}>
                    ...
                    <div className="row mx-1">
                      ...
                      {this.state.suggestionsItems.map((item, index) => (
                        <div
                          key={index}
