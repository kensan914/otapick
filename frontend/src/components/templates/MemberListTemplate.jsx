      ...
      <>
        <div className="row justify-content-between mx-2">
          <h3 className="my-auto d-flex align-items-center">
            {this.props.generation}期生
          </h3>
          <button
            onClick={() =>
              this.props.setTogglerMemory(this.props.group, this.props.index)
            }
...
class MemberListTemplate extends React.Component {
  constructor(props) {
    ...
    Object.values(GROUPS).forEach(
      (groupObj) => (this.initMembers[groupObj.id] = [])
    );
    this.initTogglerMemory = {};
    Object.values(GROUPS).forEach(
      (groupObj) => (this.initTogglerMemory[groupObj.key] = [])
    );
    this.state = {
  ...
  render() {
    ...
    Object.values(GROUPS).forEach((groupObj) => {
      if (groupObj.key === this.state.group) {
        for (const [index, members] of this.state.membersCollection[
          groupObj.id
        ].entries()) {
          membersComponent.push(
            <MemberListByGeneration
              key={`${this.state.group}-${index + 1}`}
              generation={index + 1}
              members={members}
              wavesVals={this.state.wavesVals}
              group={this.state.group}
              isOpen={this.state.togglerMemory[groupObj.key][index]}
              index={index}
              setTogglerMemory={(group, index) =>
                this.setTogglerMemory(group, index)
              }
            />
          );
