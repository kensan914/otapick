...
from api.models.main.models import Blog, Group, Member
...
def init_calc_recommend_score():
    ...
    for group_id in list(
        Group.objects.filter(is_active=True).values_list("group_id", flat=True)
    ):
        high_score_member_points = {}  # key: メンバーid, val: point
        high_score_members_list = []  # Memberオブジェクトのみ
        high_score_blogs = (
            Blog.objects.filter(writer__belonging_group__group_id=group_id)
            .exclude(score=0)
            .order_by("-score")
        )
        high_score_images = (
            Image.objects.filter(publisher__writer__belonging_group__group_id=group_id)
            .exclude(score=0)
            .order_by("-score")
        )
        ...
        for i, (high_score_blog, high_score_image) in enumerate(
            zip(high_score_blogs, high_score_images)
        ):
            ...
            if blog_tie["score"] > high_score_blog.score or i == 0:
                blog_point = (
                    len_high_score_members - i if i < len_high_score_members else 1
                )
                blog_tie["index"] = i
            else:
                blog_point = (
                    len_high_score_members - blog_tie["index"]
                    if blog_tie["index"] < len_high_score_members
                    else 1
                )
            if image_tie["score"] > high_score_image.score or i == 0:
                ...
                image_point = (
                    len_high_score_members - image_tie["index"]
                    if image_tie["index"] < len_high_score_members
                    else 1
                )
            ...
            if str(high_score_image.publisher.writer.id) in high_score_member_points:
                high_score_member_points[
                    str(high_score_image.publisher.writer.id)
                ] += image_point
            else:
                high_score_member_points[
                    str(high_score_image.publisher.writer.id)
        ...
        high_score_member_points = sorted(
            high_score_member_points.items(), key=lambda x: x[1], reverse=True
        )  # [(member_id, point), (member_id, point), ...]
        high_score_members.append(
            [
                Member.objects.get(id=member[0])
                for member in high_score_member_points[:len_high_score_members]
            ]
        )

    blogs = otapick.sort_blogs(
        Blog.objects.filter(writer__graduate=False), "older_post"
    )
    images = otapick.sort_images(
        Image.objects.filter(publisher__writer__graduate=False), "older_post"
    )
    ...
    divided_blogs = [
        Blog.objects.filter(
            id__in=blog_ids[
                int(blogs.count() * i / 3) : int(blogs.count() * (i + 1) / 3)
            ]
        )
        for i in range(3)
    ]
    ...
    divided_images = [
        Image.objects.filter(
            id__in=image_ids[
                int(images.count() * i / 3) : int(images.count() * (i + 1) / 3)
            ]
        )
        for i in range(3)
    ]
    ...
    blog_ids_graduate = list(
        Blog.objects.filter(writer__graduate=True).values_list("id", flat=True)
    )
    ...
    image_ids_graduate = list(
        Image.objects.filter(publisher__writer__graduate=True).values_list(
            "id", flat=True
        )
    )
    ...
    for high_score_members_by_G in high_score_members:
        for member in high_score_members_by_G:
            if mode == "blog":
                records = (
                    divided_records[1]
                    .filter(writer=member, recommend_score=0)
                    .order_by("?")[:5]
                )
            elif mode == "image":
                records = (
                    divided_records[1]
                    .filter(publisher__writer=member, recommend_score=0)
                    .order_by("?")[:5]
                )
            else:
                records = None
