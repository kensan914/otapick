...
from api.models.main.models import Member


class Command(BaseCommand):
    help = (
        "メンバーの宣材写真を取得。すでに登録済みのメンバーは取得しない"
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-ff", "--fforce", action="store_true", help="default:False"
        )
    ...
    def handle(self, *args, **options):
        if (
            ...
            otapick.print_console(
                "groupID {} is not supported.".format(options["group"])
            )
            quit()
        ...
        members = (
            ...
            else members.filter(belonging_group__group_id=options["group"])
        )
        for member in members:
            if (
                not member.image
                or (options["force"] and not member.graduate)
                or options["fforce"]
            ):
                ...
                if media is not None:
                    ...
                    member.save()
                    otapick.print_console(
                        "{}'s image is saved!!".format(member.full_kanji)
                    )
