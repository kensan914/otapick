...
class Command(BaseCommand):
    help = "blogのscoreを計算し、上書き。" "3時間ごとに実行。" "recommend_scoreにも対応。"
    ...
    def handle(self, *args, **options):
        if options["recommend"]:
            (
                ...
                divided_blogs,
                divided_images,
            ) = otapick.init_calc_recommend_score()

            start = time.time()
            otapick.calc_recommend_score(
                high_score_members, divided_blogs=divided_blogs
            )
            ...
            start = time.time()
            otapick.calc_recommend_score(
                high_score_members, divided_images=divided_images
            )
            otapick.print_console(
            ...
            otapick.print_console(
                "finished calc_score blogs!!: {}s".format(round(time.time() - start, 2))
            )
            start = time.time()
            otapick.calc_score(images=Image.objects.all())
            otapick.print_console(
                "finished calc_score images!!: {}s".format(
                    round(time.time() - start, 2)
                )
            )
