...
def get_upload_to(instance, filename):
    media_dir_1 = str(instance.publisher.writer.belonging_group.group_id) + \
        '_' + str(instance.publisher.writer.ct)
...
class Image(models.Model):
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['-num_of_downloads',
                                 '-recommend_score'], name='dl'),
            models.Index(
                fields=['-score', '-recommend_score'], name='popularity'),
            models.Index(
                fields=['-num_of_views', '-recommend_score'], name='view'),
        ]
        ordering = ['-publisher__post_date',
                    'publisher__order_for_simul', 'order']

    order = models.IntegerField(verbose_name='順番')
    picture = models.ImageField(
        verbose_name='イメージ(original)', upload_to=get_upload_to)
    ...
    upload_date = models.DateTimeField(
        verbose_name='アップロード日', auto_now_add=True)
    publisher = models.ForeignKey(
        Blog, verbose_name='掲載ブログ', on_delete=models.CASCADE)
    ...
    recommend_score = models.FloatField(
        verbose_name='スコア(おすすめ順）', default=False)
...
class Favorite(models.Model):
    ...
    image = models.ForeignKey(Image, verbose_name='画像',
                              on_delete=models.CASCADE)
    user = models.ForeignKey(
        Account, verbose_name='ユーザ', on_delete=models.CASCADE)
