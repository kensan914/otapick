...
class Group(models.Model):
    ...
    blog_url_format = models.CharField(
        verbose_name="公式ブログ詳細ページURIフォーマット(blog_ctを表す{}を一つ含む)",
        max_length=200,
        blank=True,
    )
    member_url_format = models.CharField(
        verbose_name="メンバー詳細ページURIフォーマット(member_ctを表す{}を一つ含む)",
        max_length=200,
        blank=True,
    )
...
class Member(models.Model):
    ...
    first_kanji = models.CharField(verbose_name="名_漢", max_length=10, blank=True)
    full_kanji = models.CharField(verbose_name="氏名_漢", max_length=20)
    last_kana = models.CharField(verbose_name="姓_かな", max_length=20)
    first_kana = models.CharField(verbose_name="名_かな", max_length=20, blank=True)
    ...
    first_eng = models.CharField(
        verbose_name="名_英", max_length=30, default="", blank=True
    )
    ...
    temporary = models.BooleanField(verbose_name="仮メンバー(ex. 新3期生)", default=False)
    is_other = models.BooleanField(verbose_name="その他メンバー(ex. ポカ)", default=False)
...
class MemberKeyword(models.Model):
    ...
    keyword = models.CharField(
        verbose_name="キーワード (ひらがな・漢字のみ使用)", max_length=100, default=""
    )
